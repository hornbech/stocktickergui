        ...
        <table class="summary-table">
          ...
          <tbody>
            @for (item of holdingItems(); track item.entry.symbol) {
              <tr class="table-row" [class.selected]="selectedSymbol() === item.entry.symbol" (click)="onSymbolSelected(item.entry.symbol)">
                ...
                <td>{{ currencyService.formatConverted(holdingItemValue(item), item.quote?.currency || 'USD') }}</td>
                <td [class.positive]="holdingPnL(item) >= 0" [class.negative]="holdingPnL(item) < 0">
                ...
                <td class="actions">
                  <button class="remove-btn" (click)="removeHolding(item.entry.symbol)" [attr.aria-label]="'Remove ' + item.entry.symbol">
          ...
          <tfoot>
            <tr class="table-footer">
              ...
              <td></td>
              <td></td>
              <td>{{ currencyService.formatDisplay(totalHoldingValue()) }}</td>
              <td [class.positive]="totalHoldingPnL() >= 0" [class.negative]="totalHoldingPnL() < 0">
